...
for mode_index in "${!MODE_SLUGS[@]}"; do
  ...
  for fixture in "${FIXTURES[@]}"; do
    stem="$slug-$fixture"
    echo "rig-render-matrix: rendering $mode / $fixture"
...
jq -s \
  ...
  --arg evidenceBoundary "ten uncropped 1280 x 720 frames from isolated real Omarchy shells; five deterministic semantic fixtures rendered in Green Monitor and Color Deluxe; each entry retains its raw shell log and rig receipt" \
  '{schemaVersion:1,generatedAt:$generatedAt,evidenceBoundary:$evidenceBoundary,
    entries:(map({fixture,displayMode:.displayModeOverride,fingerprint,sourceCommit,sourceDirty,
...
[[ "$entry_count" -eq 10 ]] || { echo "rig-render-matrix: expected 10 entries, got $entry_count" >&2; exit 1; }
...
echo "rig-render-matrix: PASS, retained 10 frames, receipts, and shell logs in $EVIDENCE_DIR"
