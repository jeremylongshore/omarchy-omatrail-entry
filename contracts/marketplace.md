...
| Green Monitor and Color Deluxe share one semantic game state for either rules profile | all presentation QML, both rules engines | profile-invariance tests in `tests/hunting.test.js` and `tests/journey.test.js` |
...
| One careful journey policy with modeled hunt results can finish both rules profiles on every difficulty | `JourneyRules.js` | 360 seeded journey-policy simulations in `tests/journey.test.js` |
