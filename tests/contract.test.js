...
test("render matrix retains both profiles across every critical scene", () => {
  ...
  assert.match(matrix, /RULE_NAMES=\(omatrail classic-1978\)/)
  ...
  assert.match(matrix, /expected 20 entries/)
  assert.match(render, /OMATRAIL_RIG_RULES_PROFILE/)
  assert.match(render, /rulesProfileOverride/)
...
test("restoring a journey preserves the selected visual mode", () => {
  ...
  assert.match(qml, /root\.rulesProfile\s*=\s*state\.rulesProfile/)
})

test("both rules choices are visible across setup, trail graphics, hunting, and endings", () => {
  const journey = read("JourneyView.qml")
  const trail = read("TrailScene.qml")
  const overlay = read("Overlay.qml")
  assert.match(journey, /label:\s*"omaTrail"/)
  assert.match(journey, /label:\s*"CLASSIC 1978-INSPIRED"/)
  assert.match(journey, /rulesProfile:\s*root\.rulesProfile/)
  assert.match(journey, /Journey\.canHunt\(root\.journeyState\)/)
  assert.match(journey, /"HUNTED HERE"\s*:\s*"HUNT"/)
  assert.match(journey, /Journey\.rulesProfileLabel\(root\.journeyState\.ending\.rulesProfile\)/)
  assert.match(trail, /Journey\.rulesProfileLabel\(root\.journeyState\.rulesProfile\)/)
  assert.match(overlay, /Journey\.rulesProfileLabel\(journey\.journeyState\.rulesProfile\)/)
})
